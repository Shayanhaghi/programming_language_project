...
;  (cond [(not (string? string_inp)) (error "first argument should be a string")] 
 ...
 ; )
(define (env_append env string_inp value)
      (append (list (cons string_inp value)) env))
    ...
    (lam "filter" "f"
                           (lam "map" "list"
                                 (cnd (ismunit (var "list")) (munit)   
                                       (ifnzero (apply (var "f") (1st (var "list"))) (apair (apply (var "f") (1st (var "list")))
...
;(eval-exp (apply (apply numex-filter-fake (lam "g" "x" (minus (num 1) (var "x"))))
 ;                 (apair (num 1) (apair (num 2) (munit)))))

;(define numex-all-gt
  ;(lam "all-gt" "value"
;            
;             (apply numex-filter-fake
;                    (lam  ">ToZero" "number" (ifleq (var "number") (var "value") (num 0)(var "number"))))  
;             
;             
;  )
;)
                       ...
                       (cond [(equal? (bool-boolean v1) #f) (bool #f)]
                             [else 
                                    (let ([v2 (eval-under-env-c (andalso-e2 e) env)])
                                     ...
                                     [(equal? (bool-boolean v2) #t) (bool #t)]
                                     [else (bool #f)])]
                                        [else (error "second argument should be a boolean")]   
                                      )
                                      )
                                    ]
                             )]
...
;(eval-exp (lam null "x" (minus (num 1) (var "x"))))
;(eval-exp

; (apply (apply numex-filter (lam null "x" (plus (num 1) (var "x")))) (apair (num 1) (apair (num 2) (munit))))
; )

;(apply ; (apair (num 1) (apair (num 2) (munit)))
; (eval-exp (list (num 1) (num 2)))
